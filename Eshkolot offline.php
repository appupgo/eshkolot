...
defined('ABSPATH') or die('No script kiddies קו!');

$childrenObj = array();
$child_index = 0;

add_shortcode('edit_details', 'edit_details');

function edit_details($atts)
{
  global $childrenObj, $child_index;
  $user_id = get_current_user_id();
  update_user_meta($user_id, 'user_type', 'private');
  $user_info = get_userdata($user_id);
  $childrenObj = set_children_obj();
  echo
    //'<script src="/wp-content/plugins/Eshkolot offline/offline.js"></script> TODO: CHANGE IN PRODUCTION!!!
    '<script src="/wp-content/plugins/eshkolot_offline/offline.js?date=' . rand() . '"></script>
    ...
    <script>
      ...
      var user_id = "' . $user_id . '";
      var year = "' . he_year() . '";
      var childrenObj = ' . json_encode($childrenObj) . ';
      var childId = ' . $child_index . ';
      var frameIndex = ' . get_frames_index() . ';
      var isEdit = "' . (get_frames_index() > 1) . '";
      var coursePerChild = {};
      var courseAllChildren = [];
    </script>';
  ...
  $html .= '<div class="close-cart" style="display: none;">
  ...
  $html .= '<div id="title_icons">
              ' . menu_icons() . '
             </div>
             <div id="edit_dtl">
             	<span>עריכת פרטים כלליים</span>
                <span>עריכת פרטי ילדים</span>
                <span>קורסים ומסלולים</span>
             </div>
    <p id="title_set">עריכת פרטים כלליים</p>
    <div id="edit_details">';
  $html .= edit_details_1();
  $html .= edit_details_2($user_info);
  $html .= edit_details_3();
  $html .= edit_details_4();
  $html .= '</div>
    <div id="forward-back-btn">
    <input id="previous_edit_details" type="button" value="→ הקודם" style="display: none;"/>
    <button id="next_edit_details">המשך ←</button></div>';
  echo $html;
}

function get_frames_index()
{
  $index = $_GET['index'];
...
function edit_details_1()
{
  $res = '<div id="edit_details_1" style="display:grid;">
              <div class="wrapper-1">
                <label>מספר הילדים במשפחה שילמדו באשכולות אופליין</label>
                <input id="num_children" type="number" value="1" min="1" />   
                <label>עיר מגורים</label>
                <input id="city_children" list="cities">
                <datalist id="cities">
...
function edit_details_2($user_info)
{
  $res = '<div id="edit_details_2" style="display:none;">
      ...
      <div class="before-table">
        <p class="child-account">חשבונות ילדים</p>
        <div id="excel">
          <a id="excelTemplate" href="/wp-content/uploads/2023/05/ילדים-אשכולות.xlsm" download>הורד קובץ אקסל לדוגמא</a>
          ...
          <input id="excelUpload" style="display:none;" type="file" accept="application/vnd.ms-excel.sheet.macroEnabled.12"/>
          </label>
        </div>
        <div class="add_child add_child_top">
...
function edit_details_3()
{
  ...
  $knowledge_space_lst .= '</dl>';
  $res = '<div id="edit_details_3" style="display: none;">
              <div class="courses_list_wrap">
                <div class="course_header">
                  <div class="active" id="all">לכל הילדים</div>
                  <div id="adapted">בהתאמה לכל ילד</div>
                </div>
                <div class="child_lst">
              ...
              <div style="display: none;" id="lbl_added">
                ...
                <label>הקורסים ו/המסלולים שבחרת נוספו לסל בהצלחה! כדי לאפשר את הורדת התוכנה תוכל לשלם כעת</label>
              </div>
            </div>';
  return $res;
}

function edit_details_4()
{
  $res = '<div id="edit_details_4" style="display: none;">
...
function polygon()
{
  $res = '<svg class="arrow" xmlns="http://www.w3.org/2000/svg" width="13" height="8" viewBox="0 0 13 8" fill="none">
            <path d="M6.5 8L0.00480952 0.5L12.9952 0.499999L6.5 8Z" fill="#2D2828"/>
           </svg>';
  return $res;
}
...
function cbx_style($id)
{
  $res = '<div style="position: relative; margin-left: 15px; display: inline-block;">
           <input class="cbx_style" id="cbx-' . $id . '" data-id="' . $id . '" type="checkbox"/>
...
function course_metadata()
{
  ...
  if ($query instanceof WP_Query) {
    $results_posts = $query->posts;
  }
  foreach ($results_posts as $id) {
    $related_course = get_post_meta($id, '_related_course', true);
    $price = get_post_meta($id, '_price', true);
    if (count($related_course) > 1) {
      array_push($learning_paths, [$id, $price]);
    } else {
      $course_id = get_post_meta($id, '_related_course', true)[0];
      $term_id = wp_get_post_terms($course_id, 'knowledge');
		$all_courses[$term_id[0]->name][] = [$id, $price, $course_id];
    }
  }
  return [$all_courses, $learning_paths];
}

function menu_icons()
{
  return '<div id="icons">
    <div class="present_icon icon_cls" data-index="1">
      <svg xmlns="http://www.w3.org/2000/svg" width="12.16" height="12.16" viewBox="0 0 13 13" fill="none">
        <path d="M0.188631 11.973L0.190865 11.9753C0.25011 12.0349 0.320567 12.0823 0.398186 12.1147C0.475805 12.147 0.559055 12.1637 0.643148 12.1638C0.713908 12.1638 0.784179 12.1521 0.851159 12.1293L4.49793 10.8922L11.4958 3.89429C11.9236 3.46645 12.164 2.88619 12.1639 2.28116C12.1639 1.67613 11.9235 1.0959 11.4957 0.668095C11.0679 0.240295 10.4876 -2.64859e-05 9.88257 2.18939e-09C9.27755 2.64902e-05 8.69731 0.240398 8.26951 0.668237L1.2716 7.66615L0.0346253 11.3128C-0.00433934 11.4262 -0.0105588 11.5483 0.01668 11.6651C0.0439189 11.7818 0.103513 11.8886 0.188631 11.973ZM8.85063 1.24931C9.12469 0.977333 9.49536 0.825043 9.88147 0.825784C10.2676 0.826526 10.6377 0.980238 10.9107 1.25326C11.1837 1.52629 11.3374 1.89637 11.3381 2.28248C11.3389 2.66859 11.1866 3.03926 10.9146 3.31332L9.99436 4.23355L7.93035 2.16954L8.85063 1.24931ZM1.98834 8.11157L7.34928 2.75061L9.41329 4.81462L4.05232 10.1756L0.928714 11.2352L1.98834 8.11157Z" fill="#2D2828"/>
      </svg>
    </div>
    <svg xmlns="http://www.w3.org/2000/svg" width="134" height="33" viewBox="0 0 135 8" fill="none">
    	<path d="M0.646454 3.64643C0.451187 3.8417 0.451187 4.15828 0.646454 4.35354L3.82843 7.53552C4.0237 7.73078 4.34027 7.73078 4.53554 7.53552C4.73079 7.34026 4.73079 7.02368 4.53554 6.82842L1.70711 3.99999L4.53554 1.17156C4.73079 0.976299 4.73079 0.659717 4.53554 0.464455C4.34027 0.269192 4.0237 0.269192 3.82843 0.464455L0.646454 3.64643ZM135 3.5L1 3.49999L1 4.49999L135 4.5L135 3.5Z" fill="black"/>
    </svg>
    <div class="icon_cls" data-index="2">
     <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 14" fill="none">
    	<path d="M12.0916 8.13683L10.3493 6.99685L11.0201 5.76703C11.1819 5.45994 11.2667 5.11814 11.2671 4.77104V2.82143C11.2685 2.27324 11.1097 1.73659 10.8103 1.27737C10.5109 0.81816 10.0839 0.456374 9.58179 0.236442C9.07965 0.0165097 8.52421 -0.0519942 7.98368 0.0393422C7.44315 0.130679 6.94106 0.377879 6.53906 0.750589L7.12795 1.3863C7.4065 1.12875 7.75415 0.958081 8.12827 0.895213C8.5024 0.832344 8.88673 0.88001 9.23416 1.03236C9.58159 1.18472 9.87702 1.43514 10.0842 1.75293C10.2914 2.07072 10.4014 2.44206 10.4006 2.82143V4.77104C10.4005 4.97316 10.3521 5.17232 10.2594 5.35192L9.20565 7.28388L11.6172 8.86182C11.7763 8.966 11.907 9.10831 11.9972 9.27581C12.0874 9.44332 12.1343 9.63071 12.1336 9.82095V11.2698H10.184V12.1363H13.0001V9.82092C13.001 9.48674 12.9183 9.15764 12.7597 8.86353C12.601 8.56942 12.3713 8.31962 12.0916 8.13683Z" fill="black"/>
    	<path d="M8.19219 9.0033L6.44992 7.86332L7.12072 6.6335C7.28252 6.32641 7.36728 5.98461 7.36772 5.63751V3.6879C7.36717 2.94061 7.07021 2.22406 6.542 1.69544C6.01379 1.16682 5.29747 0.869304 4.55018 0.868164C2.99815 0.868164 1.73551 2.13311 1.73551 3.6879V5.63751C1.73385 5.98446 1.81769 6.32646 1.97962 6.63331L2.65407 7.86979L0.921923 9.0033C0.642482 9.1858 0.413035 9.43519 0.254404 9.72883C0.0957732 10.0225 0.0129804 10.3511 0.013539 10.6848L0 13.0027H9.10071V10.6874C9.10159 10.3532 9.01893 10.0241 8.86027 9.73C8.7016 9.43589 8.47195 9.18609 8.19219 9.0033ZM8.23422 12.1362H0.871531L0.879925 10.6874C0.879271 10.4971 0.926173 10.3098 1.01637 10.1423C1.10657 9.97474 1.23719 9.83244 1.39638 9.72826L3.79771 8.15696L2.74029 6.21839C2.64862 6.03853 2.60121 5.83939 2.602 5.63751V3.6879C2.602 3.17083 2.80741 2.67494 3.17303 2.30932C3.53865 1.94369 4.03455 1.73829 4.55162 1.73829C5.06869 1.73829 5.56458 1.94369 5.9302 2.30932C6.29582 2.67494 6.50123 3.17083 6.50123 3.6879V5.63751C6.50115 5.83963 6.45273 6.03879 6.36002 6.21839L5.30625 8.15035L7.71776 9.72829C7.87695 9.83247 8.00757 9.97478 8.09777 10.1423C8.18796 10.3098 8.23487 10.4972 8.23422 10.6874V12.1362Z" fill="black"/>
    </svg>
    </div>
    <svg xmlns="http://www.w3.org/2000/svg" width="134" height="33" viewBox="0 0 135 8" fill="none">
    	<path d="M0.646454 3.64643C0.451187 3.8417 0.451187 4.15828 0.646454 4.35354L3.82843 7.53552C4.0237 7.73078 4.34027 7.73078 4.53554 7.53552C4.73079 7.34026 4.73079 7.02368 4.53554 6.82842L1.70711 3.99999L4.53554 1.17156C4.73079 0.976299 4.73079 0.659717 4.53554 0.464455C4.34027 0.269192 4.0237 0.269192 3.82843 0.464455L0.646454 3.64643ZM135 3.5L1 3.49999L1 4.49999L135 4.5L135 3.5Z" fill="black"/>
    </svg>
    <div class="icon_cls" data-index="3">
    <svg xmlns="http://www.w3.org/2000/svg" width="13" height="11.73" viewBox="0 0 13 12" fill="none">
...
function create_tranzila($fields, $details, $user_id)
{
  $expdate = $fields['exp_month'] . substr($fields['exp_year'], -2);
  $tranzila_pay = new Tranzila_Payment();
  $payment_data = array(
    'sum' => $details->total,
    'ccno' => $fields['card_number'],
    'expdate' => $expdate,
    'mycvv' => $fields['cvv_num']
  );

  $result = $tranzila_pay->create_transaction($payment_data, true);
  $result_assoc = [];
  parse_str($result, $result_assoc);
...
function create_order($fields, $user_id)
{
  global $woocommerce;
  $address = array(
  ...
  foreach ($_COOKIE as $key => $val) {
    if (strpos($key, 'shopping_offline_' . $user_id . '_') === 0) {
      $data = decode_cookie($val);
      $product_id = $data->id;
      $count = $data->count;
      $order->add_product(get_product($product_id), $count);
    }
  }
  $order->set_address($address, 'billing');
  $order->calculate_totals();
  $order->add_order_note('Tranzila Payment success. Amount received - ' . $order->get_total() . '₪');
...
function create_or_update_users($details, $user_id, $tranzila_result)
{
  ...
  if ($user_type == 'private') {
    ...
    create_or_update_students($details, $user_id);
  }
}

function get_or_create_user($child, $main_email, $last_name)
{
  $existing_child_id = get_user_by_id_number($child->id);

  if ($existing_child_id) {
    wp_update_user([
      'ID' => $existing_child_id,
      'first_name' => $child->childName,
      'last_name' => $last_name,
    ]);
    return $existing_child_id;
  }

  $pos = strpos($main_email, "@");
  $child_email = substr_replace($main_email, '+' . $child->id, $pos, 0);
  $child_id = email_exists($child_email);
  if (!$child_id) {
    $password = $child->id;
    remove_action('user_register', 'verificay_new_user');
    $child_id = wp_create_user($child_email, $password, $child_email);
    add_action('user_register', 'verificay_new_user');
    wp_update_user([
      'ID' => $child_id,
      'first_name' => $child->childName,
      'last_name' => $last_name,
    ]);
    update_user_meta($child_id, 'confirmed', true);
  }
  return $child_id;
}
...
function get_selected_courses($key, $courses)
{
  return array_merge($courses->$key->all, $courses->$key->private);
}

function create_or_update_children($details, $user_id, $tranzila_result)
{
  $children = [];
  foreach ($details->children as $key => $value) {
    $child_id = get_or_create_user($value, $details->user_email, $details->last_name);
    $children[] = ['child' => $child_id];
    $groups_ids = learndash_get_users_group_ids($child_id);
    $courses = learndash_user_get_enrolled_courses($child_id, true);
    $selected_courses = $value->paidCourses;
    $product_offline = [];
    for ($i = 0; $i < count($selected_courses); $i++) {
      $group_id = get_post_meta($selected_courses[$i], '_related_group', true)[0];
      $groups_ids[] = $group_id;
      learndash_set_users_group_ids($child_id, $groups_ids);
      $course_id = get_post_meta($selected_courses[$i], '_related_course', true)[0];
      $courses[] = $course_id;
      learndash_user_set_enrolled_courses($child_id, $courses);
      $product_offline[] = ['id' => $selected_courses[$i]];
      update_user_meta($child_id, 'refund_' . $selected_courses[$i], false);
    ...
    update_user_meta($child_id, 'city', $details->city);
    update_user_meta($child_id, 'user-id', $value->id);
		update_user_meta($child_id, 'product_offline', $product_offline);
		update_user_meta($child_id, 'paid_courses', $selected_courses);
  }
  update_user_meta($user_id, 'children', $children);

}

function write_to_slack($channel, $text){
    $slack_token = '';
    $curl = curl_init();
...
function create_or_update_students($details, $user_id)
{
  ...
  $children = [];
  $group_list = $details->groupList;
  for ($i = 0; $i < count($group_list); $i++) {
    $students_ids_list = [];
    $general_details = decode_cookie($_COOKIE['offline_group_general_details_' . $user_id . '_' . $group_list[$i]]);
    $students_details = decode_cookie($_COOKIE['offline_group_students_details_' . $user_id . '_' . $group_list[$i]]);
    $courses_details = decode_cookie($_COOKIE['offline_group_courses_details_' . $user_id . '_' . $group_list[$i]]);
    $groups[$general_details->groupName]['general_details'] = $general_details;
    $groups[$general_details->groupName]['students'] = $students_details->students;
    $all_courses = $courses_details->allCourses;
    $adapted_courses = $courses_details->adaptedCourses;
    if($general_details->payment){
      $all_courses = $courses_details->paidAllCourses;
      $adapted_courses = $courses_details->paidAdaptedCourses;
    }
    $groups[$general_details->groupName]['paidCourses']['all'] = $all_courses;
    $groups[$general_details->groupName]['paidCourses']['adapted'] = $adapted_courses;
    foreach ($students_details->students as $key => $value) {
      $student_id = get_or_create_user($value, $details->user_email, $value->last_name);
      $students_ids_list[] = $student_id;
      $children[] = ['child' => $student_id];
      $groups_ids = learndash_get_users_group_ids($student_id);
      $courses = learndash_user_get_enrolled_courses($student_id, true);
      $selected_courses = get_selected_courses($key, $adapted_courses);
      $product_offline = [];
      foreach ($selected_courses as $course) {
        $group_id = get_post_meta($course, '_related_group', true)[0];
        $groups_ids[] = $group_id;
        learndash_set_users_group_ids($student_id, $groups_ids);
        $course_id = get_post_meta($scourse, '_related_course', true)[0];
        $courses[] = $course_id;
        learndash_user_set_enrolled_courses($student_id, $courses);
        $product_offline[] = ['id' => $course];
      }
      update_user_meta($student_id, 'user-id', $value->id);
      update_user_meta($student_id, 'group-name', $general_details->groupName);
      update_user_meta($student_id, 'product_offline', $product_offline);
    }
    $groups[$general_details->groupName]['students_ids'] = $students_ids_list;
  }
  update_organization_details($user_id, $details, $groups, $children);
}

function update_organization_details($user_id, $details, $groups, $children)
{
  update_user_meta($user_id, 'organization-name', $details->organizationName);
  update_user_meta($user_id, 'organization-symbol', $details->organizationSymbol);
  update_user_meta($user_id, 'city', $details->city);
  update_user_meta($user_id, 'logo-image', $details->logoImageUrl);
  update_user_meta($user_id, 'total', $details->total);
  update_user_meta($user_id, 'groupList', $details->groupList);
  update_user_meta($user_id, 'groups', $groups);
  update_user_meta($user_id, 'children', $children);
}

function delete_cookies($user_id){
  foreach ($_COOKIE as $key => $val) {
    if (strpos($key, 'shopping_offline_' . $user_id . '_') !== false || strpos($key, 'shopping_offline_summery_' . $user_id . '_') !== false){
      unset($_COOKIE[$key]);
...
add_shortcode('offline_payment_details', 'offline_payment_details');

function offline_payment_details()
{
  $user_id = get_current_user_id();
  $user_info = get_userdata($user_id);
  return
    ...
    ' . create_summery_table($user_id);
}
function create_summery_table($user_id)
{
  $user_type = get_user_meta($user_id, 'user_type', true);
  $cookie_name = ($user_type == 'private') ? 'offline_private_details_' : 'offline_organization_details_';
  $details = decode_cookie($_COOKIE[$cookie_name . $user_id]);
  $courses = get_courses_from_cookie($user_id, $user_type);
  $courses_data = create_rows($courses);
  $html = '<div class="offline_payment_details">
  <div class="ttl">סה"כ</div>
  <div class="table-data">
  <table id="summery_table_course">
    <thead>
        <tr>
          <th>קורסים או מסלולים</th>
          <th>כמות</th>
...
<div id="return-payment-msg">החזר מוגבל עד שנה</div>
<div class="deposit">סה"כ דמי פיקדון: <label id="total-price">' . $details->total . '&nbsp;₪</label></div>
</div>';
  return $html;

}

function get_courses_from_cookie($user_id, $user_type)
{
  $cookie_name = ($user_type == 'organization') ? 'shopping_offline_summery_' : 'shopping_offline_';
  $courses_list = array();
  foreach ($_COOKIE as $key => $val) {
    if (strpos($key, $cookie_name . $user_id . '_') === 0) {
      array_push($courses_list, [$key, $val]);
    }
  }
  return $courses_list;
}

function create_rows($courses)
{
  $rows = '';
  foreach ($courses as $key => $value) {
    $data = decode_cookie($value[1]);
    $course_id = get_post_meta($value->id, '_related_course', true)[0];
    $icon = get_course_knowledge_num($data->id, $course_id);
    $rows .= '<tr><td><div>' . $data->courseName . $icon . ' </div></td>';
    $rows .= '<td>' . $data->count . ' </td></tr>';
  }
  return $rows;
}
...
function set_children_obj(){
  global $childrenObj, $child_index;
  if(in_array(get_frames_index(), [2,3])){
    $user_id = get_current_user_id();
    $children = get_user_meta($user_id, 'children', true);
    for ($child_index = 0; $child_index < count($children); $child_index++) {
      $child_id = $children[$child_index]['child'];
      $childrenObj[$child_index]['childName'] = get_user_meta($child_id, 'first_name', true);
      $childrenObj[$child_index]['age'] = get_user_meta($child_id, 'age', true);
      $childrenObj[$child_index]['id'] = get_user_meta($child_id, 'user-id', true);
      $childrenObj[$child_index]['paidCourses'] = get_user_meta($child_id, 'paid_courses', true);
      $childrenObj[$child_index]['courses'] = []; 
